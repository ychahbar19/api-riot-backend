// Validate .env file with Joi
import Joi from 'joi';
import dotenv from 'dotenv';
import { join, resolve } from 'path';

function setEnvVarsConfig() {
  ...
  const envVarsSchema = () => {
    return Joi.object({
      ...
      MONGODB_URI_DEV: Joi.string().required().description('Mongo DB host url'),
  ...
  (async function () {
    try {
      envVars = await envVarsSchema().validateAsync(process.env);
    } catch (error) {
...
export { setEnvVarsConfig, Config };
